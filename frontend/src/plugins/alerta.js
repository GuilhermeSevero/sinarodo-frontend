            ...
            if (erro.status) {
                ...
                if (erro.status !== 500) {
                    ...
                    case 422:
                        ...
                        break
                    case 400:
                        for (let prop in erro.data) {
                            if (erro.data.hasOwnProperty(prop)) {
                                if (prop !== 'general') {
                                    msg += `<br>- ${prop}: ${erro.data[prop]}`
                                } else {
                                    msg += `<br>- ${erro.data[prop]}`
                                }
                            }
                        }
                        break
                    default:
                        if (erro.data.status) {
                            msg += `<br>${erro.data.status}`
                        } else if (erro.data.general) {
                            msg += `<br>${erro.data.general[0]}`
                        }
                    }
                }
            } else {
